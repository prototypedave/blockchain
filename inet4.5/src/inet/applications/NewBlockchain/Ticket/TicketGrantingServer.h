...
#ifndef __INET4_5_TICKETGRANTINGSERVER_H_
#define __INET4_5_TICKETGRANTINGSERVER_H_
 ...
 */
class INET_API TGS : public ApplicationBase, public UdpSocket::ICallback
{
  protected:
    UdpSocket socket;
    ...
    // Block chain
    unsigned char serviceSecret[16];
    std::vector<std::string> authMessages;  // cache
  ...
  protected:
    ...
    // Block chain
    void handlePacketFromVehicle(UdpSocket *socket, Packet *packet);
    void sendPacket(std::string message, std::string vehId, std::string id); // blockchain
