...
Define_Module(BlockApp);

void BlockApp::initialize(int stage)
{
...
void BlockApp::handleMessageWhenUp(cMessage *msg)
{
    socket.processMessage(msg);
}

void BlockApp::socketDataArrived(UdpSocket *socket, Packet *pk)
{
    std::string pkname = pk->getName();
    if (pkname == "ATvtgs")
        handleAUfromServer(socket, pk);
    else if (pkname == "Transact")
        handleTransactionRequest(socket, pk);
    else
        delete pk;
}

void BlockApp::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void BlockApp::socketClosed(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void BlockApp::refreshDisplay() const
{
...
void BlockApp::finish()
{
    ApplicationBase::finish();
}

void BlockApp::handleStartOperation(LifecycleOperation *operation)
{
...
void BlockApp::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    delayActiveOperationFinish(par("stopOperationTimeout"));
}

void BlockApp::handleCrashOperation(LifecycleOperation *operation)
{
...
void BlockApp::handleAUfromServer(UdpSocket *socket, Packet *packet)
{
    auto start = std::chrono::high_resolution_clock::now();
    L3Address remoteAddress = packet->getTag<L3AddressInd>()->getSrcAddress();
    int srcPort = packet->getTag<L4PortInd>()->getSrcPort();

    auto payload = packet->removeAtFront<BlockchainPacket>();
    if (payload != nullptr)
    {
        std::string AU = payload->getAU();
        std::string vehId = payload->getVehicleId();
        std::string destId = payload->getDestination();
...
void BlockApp::handleTransactionRequest(UdpSocket *socket, Packet *pkt)
{
...
RSA * BlockApp::generatePublicKey(RSA *key)
{
